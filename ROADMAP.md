...
| Hyperbolic/flat model functions `sn_k` and `ct_k`, Definition 1.30, pp. 48--49; `morganTian2007`, `petersen2006` Ch. 9, Section 1 | `Comparison.sn`, `Comparison.ct` | A1; define the radial coefficient piecewise at `k = 0` | G1-audited A1 analytic implementation S35: scalar, traced Riccati, absolute-determinant, and abstract positive-density consumers implemented; vector/operator consumers pending |
| Positive-curvature spherical model for the upper comparison, implicit in the unnumbered analogue before `localdiffeo`, p. 49; `morganTian2007`, `petersen2016` Section 6.4, Cor. 6.4.2 and geometric cross-checks in Thms. 6.4.3/6.4.6, pp. 251--254 | `Comparison.snPos`, `Comparison.logDerivPos`, positivity and first-zero facts | A1; for `K > 0`, valid on `0 < r < pi / sqrt K`; explicit C1 input | G1-audited A1 analytic implementation S36: scalar model/Riccati/Sturm facts, direction-generic positive-density ratios, and the basis-free absolute-determinant adapter implemented; vector/operator Riccati and every geometric lower-Jacobian producer remain pending |
...
- **A1** (`G0`): hyperbolic/flat `sn`/`ct`, the positive-curvature `snPos` and
  ...
  `e874c4c7b6126984488c487cbb78077828233457`.  The finite-dimensional trace
  Cauchy--Schwarz/Riccati layer, basis-free logarithmic absolute-determinant
  formula, and direction-generic normalized positive-density adapters are
  implemented by issue #10; vector/operator Riccati remains open.  The
...
- Morgan--Tian cites Petersen's 2006 second edition.  This roadmap uses that
  ...
  6.4.3/6.4.6, pp. 251--254.  Its numbering never substitutes for a 2006
  anchor.
...
| `A1-positive-scalar-review-response` / `24afcb8519006e44b680f6bf749aa64925d8f31e` | Replaced the unrelated real-log import with the canonical logarithmic-derivative owner; moved the interval-FTC-dependent Riccati proof to `Comparison.PositiveRiccati`; made both exact-model regressions private; qualified the Sturm/Jacobi producer and Petersen attribution without changing the public comparison theorems or flat corollaries | Pinned Mathlib `Mathlib/Analysis/Calculus/LogDeriv.lean` at `520045ab14e26149ee970e2e617ca04b09bde5d6` and the exact import-set LSP probe; Petersen 2016 Section 6.4, Cor. 6.4.2 and geometric Thms. 6.4.3/6.4.6, pp. 251--254; exact diffs from `e874c4c7b6126984488c487cbb78077828233457` and review artifacts for the import, visibility, source, and module-boundary findings | Keep model profiles, origin estimates, and ODE facts independent of measure integration; import focused `Comparison.PositiveRiccati` through the Chapter 1 umbrella; retain exact-model instantiations only as private compile-time checks; in the later Jacobi bridge apply scalar Sturm on the nonvanishing interval before a hypothetical first zero and extend to the endpoint by continuity |
| `A1-trace-determinant` / issue #10, based on `2b48a6b6e6d4e115cb3d1c16e7ea7537c8bfd0f2` | Added trace Cauchy--Schwarz including dimension zero, flat/hyperbolic traced Riccati comparison, the basis-free logarithmic derivative of `|det J|`, upper/lower normalized-density monotonicity for `sn`/`snPos`, explicit flat reductions, origin-normalized consequences, equality models, and nonconstant direction regressions | Morgan--Tian `prelim.tex` Definition 1.30 and `riccurvcomp`, pp. 48--49; Petersen (2006), Chapter 9, Section 1; Petersen (2016), Section 6.4, Cor. 6.4.2, printed pp. 251--252; pinned Mathlib trace/determinant/to-matrix/continuous-multilinear and pointwise derivative APIs at `520045ab14e26149ee970e2e617ca04b09bde5d6`; reference `TraceRiccati.lean`, `MatrixCalculus.lean`, and `VolumeElement.lean` as prior art only | Keep the determinant-only API in A1 over finite-dimensional real normed spaces and require an inner product only for symmetric trace/Riccati consequences; keep matrices private; leave vector/operator Riccati, Jacobi/normal producers, positive-curvature geometric lower Jacobians, polar-volume coherence, and Bishop--Gromov to C1/C2/C3/N1 |
