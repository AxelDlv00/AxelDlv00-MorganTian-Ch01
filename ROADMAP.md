...
- **Measure and volume.** The one Riemannian volume is Mathlib's
  ...
  raw `μH` itself.  `Ch01.Metric.riemannianVolume` selects that measure over
  the original Borel structure, and the same module proves its metric,
...
| --- | --- | --- |
| `Metric` | canonical bundle metric predicates, distance/topology bridges, normalized Riemannian volume, rescaling | chart components |
