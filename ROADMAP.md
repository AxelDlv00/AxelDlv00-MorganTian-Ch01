...
laws are recorded as partial S06/S07 progress below.  The issue-44
metric-symmetry increment extends that subset: `Curvature.Symmetries` proves
metric last-pair skew and pair interchange for `ContMDiffAt ... ∞` smooth local
fields, supplies the `SmoothTensorialAt` extension/value bridge, and discharges
the selected-extension algebraic-curvature witness consumed by the direct
tangent adapters.  This smooth-local bridge does not provide the canonical
first-order (`MDiffAt`/arbitrary differentiable-extension) application theorem:
the selected-extension facade remains provisional and must not be treated as
the canonical intrinsic producer.  The raw local-frame/chart identification,
first-order producer, differential Bianchi identity, and model-coordinate
regressions remain explicit debt until their own producers are reviewed; the
first Bianchi identity is included in the proved S07 subset.
...
| `ManifoldCalculus` | pointwise smooth tangent-section vocabulary and directional-derivative regularity | none |
...
| `Curvature.Manifold` | field commutator and provisional selected-extension `(1,3)`/`(0,4)` subset | first-order (`MDiffAt`/arbitrary differentiable-extension) application, chart calculations, coordinate contractions |
| `Curvature.Tensoriality` | selected-extension pointwise tensorial and first-Bianchi subset | first-order (`MDiffAt`/arbitrary differentiable-extension) application |
| `Curvature.Symmetries` | smooth-local metric last-pair skew/pair interchange, `SmoothTensorialAt`, and selected-producer algebraic-curvature witness | first-order/`MDiffAt` canonical producer, chart/component regression, differential/second Bianchi |
...
| S06 | `(1,3)`/`(0,4)` curvature and coordinate formula: Definition 1.4, pp. 37--38; `morganTian2007` | `Curvature.Provisional.curvature`, `Curvature.Provisional.curvature4`; field commutator `Curvature.curvatureField` | F1 | S02--S03 | constant-curvature regression must verify argument order and the frozen sign convention; the selected-extension facade must be replaced at the intrinsic producer boundary | **Partial/provisional.** `Ch01.Curvature.Manifold` defines the field commutator from the exact bundled `leviCivitaConnection` and the source-ordered selected-extension `curvature4`, with first-pair skew and smooth-section regularity. `Curvature/Tensoriality.lean` adds pointwise additivity, scalar laws, and `TensorialAt` witnesses in all three `(1,3)` slots; `Curvature.Symmetries` adds a smooth-local (`ContMDiffAt ... ∞`) extension/value bridge. The first-order (`MDiffAt`/arbitrary differentiable-extension) application theorem, raw local-frame identification, Euclidean/nonconstant-metric coordinate regressions, and remaining geometric model checks are still open. Direct tangent adapters may consume the explicit selected-producer witness, but no downstream consumer may treat `Curvature.Provisional` as canonical |
| S07 | Curvature symmetries and both Bianchi identities: Claim 1.5 (`Bianchi`), p. 38; `morganTian2007` | `Curvature.Symmetries` (`curvature4Field_antisymm_last_smoothAt`, `curvature4Field_pair_swap_smoothAt`, `curvatureField_eq_of_value_eq`, and `Provisional.curvature4_isAlgebraicCurvature`) plus the existing first-Bianchi declarations | F1 | S06 and tensor covariant derivative | permutations and second-derivative regularity must match the chosen `(0,4)` order | **Partial, smooth-field symmetry increment.** The exact bundled Levi--Civita field layer proves metric last-pair skew and pair interchange for explicitly smooth local sections, and its `SmoothTensorialAt` adapter proves smooth extension independence at a point. The selected-extension four-tensor now has an automatic full algebraic-curvature witness and pair-interchange theorem. These results are restricted to the smooth-local contract; the weaker first-order (`MDiffAt`/arbitrary differentiable-extension) canonical producer, public chart/component bridge and manifold regression, and differential/second Bianchi identity remain open S07 work. |
| S08 | Sectional curvature, constant-curvature tensor identity, spherical/Euclidean/hyperbolic examples, and curvature operator: Definitions 1.6--1.7 and intervening model paragraph, pp. 38--39; `morganTian2007` | `Curvature.Sectional` (`sectionalCurvature`, `HasConstantCurvature`), `Curvature.Plane` (`SectionalPlane`, `sectionalCurvaturePlane`), and `Curvature.Operator` (`curvatureOperator`); direct-only `Curvature.SectionalProvisional` (`sectionalCurvatureAt`, `sectionalCurvatureAtPlane`, `IsAlgebraicCurvatureAt`) and `Curvature.OperatorProvisional` (`curvatureOperatorAt`); `Models.standardSpaceCurvature` | F1 -> F3 | S06--S07 and exterior-square API; F1 curvature API for F3 models | fix orthonormal-pair independence and wedge normalization before positivity claims; standard models are sign regressions, not assumed examples | **Partial intrinsic-plane/algebraic/operator boundary.** The generic plane and exterior-square APIs remain implemented as before. `Curvature.Symmetries` now supplies the selected producer's full algebraic-curvature witness, so the direct tangent adapters can be discharged with `Provisional.curvature4_isAlgebraicCurvature` while retaining their explicit witness argument for compatibility. The first-order canonical tangent producer, independent nonconstant-metric Levi--Civita regression, canonical orthonormal-plane choices, and standard spherical/hyperbolic manifold constructions remain open. |
...
| Selected-extension `Curvature.Provisional.curvature`/`curvature4` facade | S06/S07 exploration plus direct tangent witness consumers; smooth-local `ContMDiffAt ... ∞` tensoriality and extension/value independence are proved, but the `FiberBundle.extend` producer remains noncanonical and carries a maintenance/import cost | Replace or retire it when a first-order (`MDiffAt`/arbitrary differentiable-extension) application theorem identifies the canonical producer and its field values. The direct tangent adapters may continue to consume the explicit selected-producer witness until migration; canonical downstream geometry remains gated on this trigger |
...
| `issue-44-metric-symmetries` / `d07618bb84db85a26c01c4791448f4e4e418975f` | Added `ManifoldCalculus` as the shared owner of pointwise smooth tangent-section and directional-derivative regularity, plus `Curvature.Symmetries` and its scalar-bracket-commutator leaf: the exact bundled Levi--Civita field proves metric self-contraction zero, last-pair skew, pair interchange, smooth local tensoriality, and smooth-extension independence; the selected `Provisional.curvature4` now has an automatic full algebraic-curvature witness consumed by the tangent adapters. The first-order canonical producer remains explicitly separate | Morgan--Tian Claim 1.5, printed pp. 37--38 (`morganTian2007`); do Carmo Chapter 4, Section 3 (`doCarmo1992`); Petersen Chapter 3 (`petersen2016`); pinned Mathlib metric-compatibility, Hom-bundle derivative regularity, Lie-bracket/Jacobi, `FiberBundle.extend`, and `TensorialAt` APIs; fresh focused elaboration of the shared calculus and changed curvature chain, Chapter 1 root import, exported `#print axioms`, and forbidden-import/proof-hole/hygiene scans | Mark the smooth-field metric-symmetry and selected-witness subset implemented. Keep S07/S08 partial: the weaker first-order arbitrary-extension producer, public component/nonconstant-metric regression, canonical orthonormal-plane choices, differential/second Bianchi identity, and standard sphere/hyperbolic constructions remain open |
