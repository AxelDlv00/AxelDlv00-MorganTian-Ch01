...
facade is explicitly provisional: it has no first-order arbitrary
smooth-extension application theorem and must not be treated as the canonical
intrinsic producer or consumed by downstream geometry.  The issue-55 smooth-field
increment proves metric last-pair skew and pair interchange for fields smooth at
the evaluation point, a smooth extension/value bridge, and a source-ordered
differential Bianchi adapter for globally smooth fields.  These `SmoothAt` and
`CMDiff ∞` results are compatibility bridges only: the first-order Mathlib
`TensorialAt`/rank-generic producer and its arbitrary-extension application
theorem remain the replacement gate.  Raw chart identification,
model-coordinate regressions, and the other named producer checks remain
explicit debt; the first Bianchi identity is included in the proved S07 subset.
...
| `Curvature.Manifold` | field commutator and provisional selected-extension `(1,3)`/`(0,4)` subset | first-order producer/application, chart calculations, coordinate contractions |
| `Curvature.Tensoriality` | selected-extension pointwise tensorial and first-Bianchi subset | first-order producer/application boundary and compatibility plumbing |
| `Curvature.Symmetries` | smooth-at metric last-pair skew and pair interchange, `SmoothTensorialAt` extension/value bridge, and globally smooth source-ordered differential-Bianchi adapters | first-order Mathlib `TensorialAt`/rank-generic producer, arbitrary-extension application, and chart/component regressions |
...
| Selected-extension `Curvature.Provisional.curvature`/`curvature4` facade | S06/S07 exploration only; fiberwise laws, smooth-at symmetry bridges, and first Bianchi are useful checks, but the `FiberBundle.extend` producer has no first-order arbitrary-extension application theorem and carries a maintenance/import cost | Replace or retire it when a first-order Mathlib `TensorialAt`/rank-generic producer in all field slots is paired with the arbitrary smooth-extension application theorem; the smooth `ContMDiffAt`/`CMDiff ∞` bridges alone do not fire this trigger, and downstream curvature consumers remain gated until it is met |
...
| `issue-55-smooth-curvature-symmetry` / reviewed head `5c63246ef67385268fe506dcd75286113f5a80b6` | Added the `Curvature.Symmetries` public boundary: smooth-at metric last-pair skew and pair interchange, the `SmoothTensorialAt` extension/value bridge, and the source-ordered differential Bianchi adapter; exported the focused module and retained witness-gated selected-extension tangent consumers. The revision also records the explicit smooth-versus-first-order producer distinction and the `Curvature.Symmetries` ownership boundary | Morgan--Tian Claim 1.5 (`morganTian2007`), retained arXiv printed pp. 37--38; pinned Mathlib `CovariantDerivative/{Basic,Metric,Torsion}`, `ContMDiffSection`, `Tensoriality`, and `VectorField.LieBracket` APIs at `520045ab14e26149ee970e2e617ca04b09bde5d6`; direct changed-leaf/root elaboration, exported signature and axiom probes, and source/import/hole scans | Keep S07 partial. The `SmoothAt` metric symmetries and globally `CMDiff ∞` differential-Bianchi adapter are compatibility bridges, not the first-order Mathlib `TensorialAt`/rank-generic producer or its arbitrary smooth-extension application theorem. Retain the selected-extension facade and explicit `IsAlgebraicCurvatureAt` witness until that producer/application trigger is met; keep `Curvature.Symmetries` as the smooth-field owner, leave chart/component and manifold model regressions open, and do not migrate downstream consumers at this revision. The roadmap's same-PR audit rule controls over issue #55's narrower S07-row-only wording because this revision changes a public import and ownership boundary |
