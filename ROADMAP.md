...
| S07 | Curvature symmetries and both Bianchi identities: Claim 1.5 (`Bianchi`), p. 38; `morganTian2007` | `Curvature.Symmetries` (`curvature4Field_antisymm_last_smoothAt`, `curvature4Field_pair_swap_smoothAt`, `curvatureField_eq_of_value_eq`, and `Provisional.curvature4_isAlgebraicCurvature`) plus the existing first-Bianchi declarations | F1 | S06 and tensor covariant derivative | permutations and second-derivative regularity must match the chosen `(0,4)` order | **Partial, smooth-field symmetry increment.** The exact bundled Levi--Civita field layer proves metric last-pair skew and pair interchange for explicitly smooth local sections, and its `SmoothTensorialAt` adapter proves smooth extension independence at a point. The selected-extension four-tensor now has an automatic full algebraic-curvature witness and pair-interchange theorem. The weaker first-order arbitrary-extension producer, public chart/component bridge and manifold regression, and differential/second Bianchi identity remain open S07 work. |
