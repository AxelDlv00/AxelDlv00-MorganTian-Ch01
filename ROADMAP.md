...
| S19 | On a complete boundaryless manifold, minimizing geodesics and the Hopf--Rinow implication: paragraph before and Theorem 1.18, pp. 41--42; `morganTian2007`; `doCarmo1992`, Ch. 7, pp. 157--166; `lee2018`, Thm. 6.19 | `Geodesic.IsGeodesicallyComplete`, `Geodesic.exists_minimizingGeodesic`, `hopfRinow` | F2 | S01, metric completeness, and the all-initial-data `[BoundarylessManifold I M]` branch of S18 | keep metric completeness and boundarylessness as independent public assumptions, and connect them to the canonical maximal geodesic and minimizer existence, with connected-component assumptions explicit | **Partial.** `Ch01.Geodesic.HopfRinow` exports the canonical selected-distance completeness predicate and an explicit bridge reinstating its exact `EMetricSpace`/`CompleteSpace M` pair; maximal lifetimes are open, and continuation contracts quantify open order-connected (interval-shaped) superdomains while carrying `[CompleteSpace E]` plus `[BoundarylessManifold I M]`. It also exports proved unbounded-lifetime/all-real-time consequences, a source-facing `IsGeodesicallyComplete` wrapper consuming the named maximal-domain producer, a global-witness-to-maximal producer adapter, the quadratic connection-contraction law and proved affine/time-reversal/translation equation transport, affine path-length/restriction/translation compatibility, and the proof-backed finite subsegment length theorem and `restrict_of_finite` adapter under an explicit finite selected distance. The core minimizer constructor still accepts a genuine length certificate for producer compatibility. Proof-backed subsingleton producer adapters now instantiate the all-real-time and finite-distance branches in a zero-dimensional Euclidean regression, including the selected-metric completeness witness and an explicit unbounded-lifetime check. Canonical Euclidean length/speed regressions (including dimensions zero and one), component-local finite-path/finite-distance witnesses and minimizer adapters, concrete bounded-lifetime and disconnected continuation-domain negative controls (including the `⊤` clopen-component distance theorem), and a signature-level local-IVP probe under `¬ RiemannianMetricComplete` remain exported. The actual S18 continuation and compactness/minimizer producers remain explicit interfaces (`CompleteMaximalGeodesicData`, `MinimizingGeodesicData`); no rejected dependency facade or proof hole is used. |
...
| `issue-42-universal-maximal-witness` / `536e2a2` | Added the universal `AllMaximalGeodesicContinuation` contract and proof-backed all-maximal global/lifetime theorem, a subsingleton arbitrary-witness regression, and the finite-subsegment candidate-versus-competitor path-length inequality; retained the explicit S18 continuation and compactness/minimizer producer interfaces | Morgan--Tian's complete-manifold paragraph before Theorem 1.18 (`morganTian2007`); do Carmo Chapter 7 (`doCarmo1992`); Lee Theorem 6.19 (`lee2018`); pinned `MaximalGeodesic.maximal`, `ContinuousOn`/`isGeodesicOn` interval APIs, and `Manifold.riemannianEDist_le_pathELength`/path-length additivity | Keep S19 **Partial**. The universal theorem quantifies over every supplied maximal witness but does not construct the unresolved S18 continuation producer; the minimizer constructor still consumes a genuine compactness/length certificate. Preserve the separate selected-metric, model-space, and boundaryless hypotheses, and do not add uniqueness or no-conjugate claims |
...
| `issue-42-component-paths` / `0f39636` | Added the component-local finite piecewise-smooth path witness and its finite path-length, canonical Riemannian-distance, and `C^1` adapters; added generic and selected-metric right-endpoint Cauchy bridges under an explicit extended-distance Lipschitz premise; added component-local minimizing-geodesic wrappers that consume the existing explicit compactness producer; retained the global preconnected specializations and the disconnected-component negative distance regression | Morgan--Tian's complete-manifold paragraph before Theorem 1.18 (`morganTian2007`); do Carmo Chapter 7 (`doCarmo1992`); Lee Theorem 6.19 (`lee2018`); pinned Mathlib `Topology.Connected.Clopen` (`IsClopen.connectedComponent_subset`), `Geometry.Manifold.Riemannian.PathELength` (`riemannianEDist_le_pathELength`), and `Topology.EMetricSpace` Cauchy APIs | Keep S19 **Partial**. Component-local finite paths now expose the exact connected-component hypothesis without asserting path-connectedness, metric completeness, maximal continuation, or compactness. The endpoint theorem remains a right-endpoint analytic ingredient whose geodesic speed premise and left adapter are future producer work. The minimizer wrappers remain producer-consuming; positive-dimensional Euclidean geodesic packaging and the general S18 continuation/minimizer producers remain open. |
