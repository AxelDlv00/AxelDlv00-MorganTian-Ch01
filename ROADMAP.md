...
- **Geodesics and exponential.** G2 selects no candidate geodesic API.
  `MorganTianLib.Ch01.Geodesic` owns the project-built target API in F2; the
  current chart-local implementation is a direct-only staging leaf until S18's
  producer/equivalence boundary is proved.
  The maximal exponential domain preserves an unbounded/complete case rather
  than using a finite-radius facade.
- **Jacobi fields.** The target `JacobiField gamma` is an intrinsic
  tangent-bundle field along `gamma` satisfying the source-ordered covariant
  Jacobi equation `D^2 J + R J V V = 0`.  The current `Ch01.Jacobi` leaf is
  direct-only: its chart certificates and selected-extension curvature remain
  staging data until the explicit S06/S18/S21 equivalence triggers land.  Chart
  `(J,DJ)` pairs and parallel-frame matrices are private proof representations
  for the final API, with explicit equivalence theorems still required.
...
- **Index form.** The target public index form is the intrinsic symmetric
  bilinear integral on fields along a geodesic, with endpoint conditions stated
  explicitly.  The current `Ch01.Geodesic.IndexForm` leaf is direct-only
  analytic staging over the selected-extension curvature facade.  A
  frame/inner-product-space form is an adapter proved equal to the final
  intrinsic form, never a replacement definition.
...
| `Geodesic` | target IVP/maximal-domain and `exp` API; current chart-local direct-only staging | complete-only helpers and chart reductions |
| `Jacobi` | target intrinsic fields, equation, existence/uniqueness, conjugacy, `dExp` bridge; current direct-only certificate leaf | chart pairs, frames, selected-extension curvature |
| `IndexForm` | target intrinsic index form, second variation, minimizer criterion; current direct-only analytic assembly | frame quadratic form and selected-extension producer |
...
| S18 | Geodesic equation, coordinate ODE, IVP uniqueness/smoothness: Definition 1.17 and following paragraph, p. 41; `morganTian2007`; `doCarmo1992`, Ch. 3, pp. 61--75 | `Geodesic.isGeodesic`, intrinsic IVP/maximal solution | F2 | S02; `[CompleteSpace E]` for the checked ODE API; `[BoundarylessManifold I M]` for the all-initial-data contract, or `I.IsInteriorPoint p` for a point-local IVP | prove chart ODE/intrinsic equivalence, interval maximality, and smooth dependence; export the weakest manifold-level/interior-point hypothesis rather than `[I.Boundaryless]`; a pointwise equation is insufficient | **Open; direct-only staging.** `Ch01.Geodesic` supplies chart-local ODE, moving-foot acceleration, and conditional local uniqueness certificates, but no chart-independence or maximal/smooth-family producer. It is absent from the stable umbrella and is prior art for the accepted S18 contract, not its completion. |
| S19 | On a complete boundaryless manifold, minimizing geodesics and the Hopf--Rinow implication: paragraph before and Theorem 1.18, pp. 41--42; `morganTian2007`; `doCarmo1992`, Ch. 7, pp. 157--166; `lee2018`, Thm. 6.19 | `Geodesic.exists_minimizingGeodesic`, `hopfRinow` | F2 | S01, metric completeness, and the all-initial-data `[BoundarylessManifold I M]` branch of S18 | keep metric completeness and boundarylessness as independent public assumptions, and connect them to the canonical maximal geodesic and minimizer existence, with connected-component assumptions explicit | **Open; direct-only staging.** `Geodesic.HopfRinow` keeps metric completeness, boundarylessness, continuation, and minimizer existence as separate named premises and proves their conditional set-theoretic assembly. It is absent from the stable umbrella until S18/S19 provide the geometric producers; the rejected dependency's facade is not evidence. |
...
| S21 | Geodesic variations, Jacobi equation, and initial-data uniqueness: pp. 43--44; `morganTian2007`; `doCarmo1992`, Ch. 5, pp. 101--121 | `Jacobi.JacobiField`, variation bridge, existence/uniqueness | J1 | F1 and F2 | intrinsic equation `D^2 J + R J V V = 0` must agree with chart/frame adapters and frozen curvature order | **Open; direct-only staging.** `Ch01.Jacobi` supplies source-safe local-window chart certificates, algebraic initial-data laws, and an explicit variation commutation record. Its Jacobi equation still consumes `Curvature.Provisional`, so it is absent from the stable umbrella pending the S06 arbitrary-extension application theorem and the S18 chart-independent equivalence/producer. Existence, uniqueness, and the source-facing intrinsic API remain open. |
| S22 | Conjugate point: Definition 1.19, p. 44; `morganTian2007` | `Jacobi.IsConjugate` | J1 | S21 | quantify a nonzero intrinsic field vanishing at the initial and target endpoints | **Open; direct-only staging.** The current `IsConjugate` packages nonzero and endpoint-zero conditions over the staged Jacobi certificate, but it is not a stable source-facing declaration until S21 crosses the same curvature and chart-independence gates. |
| S23 | Full second variation, boundary term, bilinear form, and endpoint-fixed index form: pp. 43--45; `morganTian2007`; `doCarmo1992`, Ch. 9, pp. 185--201; `lee2018`, Thm. 10.22 and Prop. 10.24 | `IndexForm.indexForm`, `secondVariation` | V1 | S20--S21 | select and prove sufficient regularity for arbitrary families; do not silently drop the free-end boundary term | **Partial analytic/direct-only staging.** `Ch01.Geodesic.IndexForm` uses Mathlib's `NonemptyInterval ℝ` and supplies the source-ordered selected-extension `jacobiOperator`/`curvaturePairing`/`indexForm`, source-safe local-window derivative obligations, FTC integration by parts, endpoint-zero `- integral <Jac(Y1),Y2>`, bilinearity, and conditional symmetry. `SecondVariationCore` and the `SmoothVariation`/`GeodesicVariation`-anchored records expose the mixed derivative, density bridge, base-geodesic and transverse endpoint-acceleration witnesses, and both free-end signs; fixed-end corollaries retain the separate endpoint-acceleration obligation. Zero/flat-affine/constant-curvature, metric-component sign, and zero/one-dimensional model regressions remain useful direct-leaf checks. The leaf is absent from the stable umbrella because its curvature is `Curvature.Provisional` and its derivative/geodesic certificates are chart based. Promotion requires the S06 arbitrary-extension application theorem and S18/S21 chart-independent producer/equivalence. The arbitrary-family C3 producer, derivation of the density/mixed-partial bridge from the canonical connection, a non-opaque transverse-acceleration endpoint theorem, and geometric positive-affine field transport remain open; this does not complete V1, S24, or S25. |
...
| Direct-only geodesic/Jacobi/index-form staging leaves (`Geodesic`, `Geodesic.HopfRinow`, `Jacobi`, `Geodesic.IndexForm`) | Focused chart-local certificates and analytic assembly only; they are absent from `Ch01.lean`, and the Jacobi/index operators consume selected-extension curvature rather than a canonical producer | Promote only after S18 supplies the chart-independent geodesic producer/equivalence and S06 supplies the arbitrary smooth-extension curvature application theorem; migrate S21/S23 consumers before stable umbrella export |
...
| `issue-41-regularity-and-frame-sign` / `9e0cd52` | Added an evaluation-level producer for arbitrary finite mixed ranks under an indexed metric-dual regularity witness, the witness-free covariant (`p = 0`) specialization with scalar rank-zero and one-form adapters, a Hom-bundle directional-derivative regularity bridge, and local-frame inverse-Gram coefficients that discharge the canonical frame covector's first-order metric-dual premise and expose the dual-action `-Γ` sign | Morgan--Tian discussion preceding `lapformula`, pp. 39--40, `morganTian2007`; pinned Mathlib `ContMDiffMFDeriv`/Hom-bundle regularity, `CovariantDerivative.Metric`, local-frame coefficient and Gram/determinant APIs, and the existing `Connection.covariantVector_christoffel_formula`; focused pinned-artifact elaboration, exported-signature/axiom checks, and hole/import scans | Keep S12 partial and direct-only. The unconditional mixed producer, full tensor-product section construction, canonical Hessian/trace compatibility, and flat/nonconstant model regressions remain open; the lower-level generic-frame bridge retains a local differentiability premise, while the canonical frame wrapper discharges it on the base set via inverse-Gram coefficients |
...
| `issue-56-index-form-staging` / reviewed head `af8cdcf48c4546975de46118c134c4a9736a6191` | Added direct-only staging leaves `Ch01.Geodesic`, `Geodesic.HopfRinow`, `Ch01.Jacobi`, and `Geodesic.IndexForm`, including chart/moving-foot geodesic certificates, Hopf--Rinow producer records, source-safe Jacobi and conjugacy contracts, and selected-extension Jacobi/index-form/second-variation assembly. Removed all four leaves from the stable `MorganTianLib/Ch01.lean` umbrella and recorded the S18/S19/S21/S22/S23 staging boundaries | Rechecked Morgan--Tian Chapter 1 pp. 41--45 (`morganTian2007`), do Carmo Chapters 3/5/7/9 (`doCarmo1992`), Lee Theorems 6.19 and 10.22 and Proposition 10.24 (`lee2018`), pinned Mathlib `CovariantDerivative`, `ExistUnique`, `NonemptyInterval`, bounds, `PathELength`, and interval FTC/integration-by-parts APIs at `520045ab14e26149ee970e2e617ca04b09bde5d6`, plus exact-head import/proof-hole/axiom/unsafe/forbidden-import scans | Keep all four leaves direct-only and absent from `Ch01.lean`; retain S18/S19/S21/S22 as open and S23 as partial. Promote only after S06's arbitrary-extension curvature theorem and S18's chart-independent producer/equivalence land, then migrate S21/S23 consumers. Retain open maximality, minimizer, Jacobi existence/uniqueness, arbitrary-family, endpoint-acceleration, and transport debt; make no S24/S25 or downstream completion claim. Before promotion, move public chart adapters behind the intrinsic boundary or a clearly named compatibility leaf |
