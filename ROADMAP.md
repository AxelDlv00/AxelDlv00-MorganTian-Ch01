...
`Ch01.Curvature.Model` supplies the algebraic sign/order kernel and five
regressions, while `Ch01.Curvature` owns the dependent manifold commutator.
...
whole source claim: the exact bundled Levi--Civita commutator, its source-ordered
four-tensor, local-frame coordinate calculation, and pointwise slot laws are
recorded as partial S06/S07 progress below.  The extension/local-frame chart
...
The current F1/F2/J1 contributor slice is a coordinated integration of those
partial substrate interfaces, not a completion of the sibling milestones.  The
connection-free algebraic sign kernel is owned by `Curvature.Model`, the
manifold commutator remains in `Curvature`, and the interval predicate
`Geodesic.isGeodesicOn` is owned by the lower geodesic layer so `Jacobi` does not
depend on Hopf--Rinow.  `Geodesic.HopfRinow` and `Jacobi` consume these interfaces
through explicit producer records and source-constrained certificates; S18/S19
continuation, compactness, chart transfer, and ODE producers remain open.  Open
sibling producer work is therefore treated as coordination/prior-art input,
not as an implicit second public implementation or as evidence that these rows
have closed.
...
it retains the action's hosted Mathlib artifact restoration and explicitly
...
- **Curvature.** The public `(1,3)` tensor is `R X Y Z` with
  ...
  before J1.  The connection-free algebraic declarations live in
  `Ch01.Curvature.Model`; the manifold commutator and its field-level bridges
  live in `Ch01.Curvature` and depend on the canonical connection.
- **Geodesics and exponential.** G2 selects no candidate geodesic API.
  ...
  Its lower-level `isGeodesicOn` interval contract is also the dependency leaf
  for Jacobi; completeness and minimizer adapters remain in
  `Geodesic.HopfRinow`.
...
- **Jacobi fields.** `JacobiField gamma` is an intrinsic tangent-bundle field
  ...
  source-constrained `JacobiField`/`IsGeodesicJacobiFieldOn` certificate and its
  fixed-chart algebra; the
...
| `Curvature.Model` | connection-free constant-curvature operator/four-tensor and sign regressions | model-specific coordinates |
| `Curvature` | `curvature`, `(0,4)` form, `sectionalCurvature`, `ricci`, scalar, Bianchi, pullback naturality | coordinate contractions and local-frame adapters |
| `Models` | `constantCurvature`, Einstein consequences, `coneMetric`, cone curvature | model-specific coordinates |
| `Geodesic` | IVP/maximal domain, interval `isGeodesicOn`, speed/energy, first variation, `exp` | chart coefficients and complete-only helpers |
| `Geodesic.HopfRinow` | conditional completeness, continuation, and minimizing-segment interfaces | producer records and complete-only adapters |
...
| S06 | `(1,3)`/`(0,4)` curvature and coordinate formula: Definition 1.4, pp. 37--38; `morganTian2007` | `Curvature.Model`, `Curvature.curvature`, `Curvature.curvature4` | F1 | S02--S03 | constant-curvature regression must verify argument order and the frozen sign convention | **Partial.** `Ch01.Curvature.Model` owns the connection-free constant-curvature operator/four-tensor and sign regressions. `Ch01.Curvature` defines the extension-based commutator from the exact bundled `leviCivitaConnection`, the source-ordered `curvature4`, first-pair skew, smoothness for smooth local sections, and the local-frame chart formula with the stated derivative/quadratic Christoffel order. `Curvature.Tensoriality` adds pointwise additivity, scalar laws, and `TensorialAt` witnesses in all three `(1,3)` slots. The theorem identifying raw local-frame components with the extension-based pointwise value, Euclidean/nonconstant-metric coordinate regressions, and the remaining geometric model checks are still open |
| S07 | Curvature symmetries and both Bianchi identities: Claim 1.5 (`Bianchi`), p. 38; `morganTian2007` | Current subset: `Curvature.curvature_swap`, `curvature4_swap_first`, `Curvature.Tensoriality.curvature_tensorial_*`, `Curvature.Tensoriality.curvature_bianchi`, and `curvature4_bianchi`; target families `Curvature.symm`, `bianchi1`, `bianchi2` | F1 | S06 and tensor covariant derivative | permutations and second-derivative regularity must match the chosen `(0,4)` order | **Partial.** The first-pair skew law, the three-slot tensorial additivity/scalar subset, and the extension-based first Bianchi identity in both `(1,3)` and source-ordered `(0,4)` forms are exported (including `TensorialAt` witnesses). Metric last-pair skew, pair interchange, and the differential/second Bianchi identity remain open; no full symmetry or second-Bianchi completion is claimed |
| S08 | Sectional curvature, constant-curvature tensor identity, spherical/Euclidean/hyperbolic examples, and curvature operator: Definitions 1.6--1.7 and intervening model paragraph, pp. 38--39; `morganTian2007` | `Curvature.sectionalCurvature`, `constantCurvature`, `curvatureOperator`; `Models.standardSpaceCurvature` | F1 -> F3 | S06--S07 and exterior-square API; F1 curvature API for F3 models | fix orthonormal-pair independence and wedge normalization before positivity claims; standard models are sign regressions, not assumed examples | **Partial.** `Ch01.Curvature.Model` supplies only the algebraic constant-curvature model and its orthonormal-pair sign regression; geometric sectional curvature, curvature operators, and standard-space examples remain open |
| S09 | Ricci and scalar curvature: Definition 1.8, p. 39; `morganTian2007` | `Curvature.ricci`, `scalarCurvature` | F1 | S06--S07, the F1 curvature API in S08, and finite-dimensional trace | contractions must reproduce the constant-curvature values with the frozen slot order | **Open geometric API.** The required second/fourth-slot algebraic contraction regression is proved in `Ch01.Curvature.Model`, including dimensions zero and one; manifold Ricci and scalar curvature remain open |
...
| S18 | Geodesic equation, coordinate ODE, IVP uniqueness/smoothness: Definition 1.17 and following paragraph, p. 41; `morganTian2007`; `doCarmo1992`, Ch. 3, pp. 61--75 | `Geodesic.isGeodesic`, intrinsic IVP/maximal solution | F2 | S02; `[CompleteSpace E]` for the checked ODE API; `[BoundarylessManifold I M]` for the all-initial-data contract, or `I.IsInteriorPoint p` for a point-local IVP | prove chart ODE/intrinsic equivalence, interval maximality, and smooth dependence; export the weakest manifold-level/interior-point hypothesis rather than `[I.Boundaryless]`; a pointwise equation is insufficient | **Open (integration interface only).** `Ch01.Geodesic` supplies the chart-local equation, intrinsic point predicate, and lower-level interval restriction contract consumed by J1/F2 adapters. Moving-chart equivalence, interval maximality, smooth dependence, and the all-initial-data producer remain open; this revision does not promote S18 to complete |
| S19 | On a complete boundaryless manifold, minimizing geodesics and the Hopf--Rinow implication: paragraph before and Theorem 1.18, pp. 41--42; `morganTian2007`; `doCarmo1992`, Ch. 7, pp. 157--166; `lee2018`, Thm. 6.19 | `Geodesic.exists_minimizingGeodesic`, `hopfRinow` | F2 | S01, metric completeness, and the all-initial-data `[BoundarylessManifold I M]` branch of S18 | keep metric completeness and boundarylessness as independent public assumptions, and connect them to the canonical maximal geodesic and minimizer existence, with connected-component assumptions explicit | **Partial interface only.** `Ch01.Geodesic.HopfRinow` consumes the lower-level `Geodesic.isGeodesicOn` contract and exports explicit maximal-lifetime/continuation and minimizing-segment producer records, plus the proved unbounded-lifetime, restriction/translation, and zero-length adapters. The actual continuation, compactness, and minimizer producers remain conditional interfaces (`CompleteMaximalGeodesicData`, `MinimizingGeodesicData`); no scratch completeness/component probes or hidden existence theorem is claimed. |
...
| S21 | Geodesic variations, Jacobi equation, and initial-data uniqueness: pp. 43--44; `morganTian2007`; `doCarmo1992`, Ch. 5, pp. 101--121 | `Jacobi.JacobiField`, variation bridge, existence/uniqueness | J1 | F1 and F2 | intrinsic equation `D^2 J + R J V V = 0` must agree with chart/frame adapters and frozen curvature order | **Partial.** `Ch01.Jacobi` now exports dependent tangent-bundle fields, the source-constrained `JacobiField`/`IsGeodesicJacobiFieldOn` contract, source-ordered curvature/derivative certificates on interval windows, fixed-chart zero/add/scalar laws, restriction and zero checks, an ordered initial-data map, and `GeodesicVariation` with explicit smoothness, slice-geodesic, variational-`mfderiv`, and commutation fields. The canonical model-sign regressions live in `Curvature.Model`; chart transfer, smooth-family-to-Jacobi proof, initial-data ODE existence/uniqueness, cross-chart linearity, and affine transport remain open continuation work. This is a coordinated consumer of the partial F1/F2 interfaces, not a completion of those milestones |
...
| S23 | Full second variation, boundary term, bilinear form, and endpoint-fixed index form: pp. 43--45; `morganTian2007`; `doCarmo1992`, Ch. 9, pp. 185--201; `lee2018`, Thm. 10.22 and Prop. 10.24 | `IndexForm.indexForm`, `secondVariation` | V1 | S20--S21 | select and prove sufficient regularity for arbitrary families; do not silently drop the free-end boundary term | **Partial.** `Ch01.Geodesic.IndexForm` exports the named compact `ClosedInterval`, source-ordered intrinsic `jacobiOperator`/`curvaturePairing`/`indexForm`, the FTC integration-by-parts identity, endpoint-zero `- integral <Jac(Y1),Y2>`, additivity and scalar laws in both fields, and conditional symmetry under an explicit `CurvatureSelfAdjoint` witness. `SecondVariationCore` and the `SmoothVariation`/`GeodesicVariation`-anchored data records expose the mixed derivative, density bridge, base-geodesic and transverse endpoint-acceleration witnesses, and both free-end signs; fixed-end corollaries retain the separate endpoint-acceleration obligation. Public zero/flat-affine/constant-curvature, metric-component sign, and zero/one-dimensional model regressions are included. The arbitrary-family C3 producer, derivation of the density/mixed-partial bridge from the canonical connection, a non-opaque transverse-acceleration endpoint theorem, and geometric positive-affine field transport remain open; the conditional assembly is not claimed as the full V1 completion. |
...
- **G2** (`G1`, human gate; complete at
  ...
  `Ch01.Curvature.Model` implement the algebraic/sign portion of those families;
  the dependent manifold `Ch01.Curvature` module is an F1 consumer.  All are
  imported by the Chapter 1 umbrella.  The ledger's geodesic-equation row is a post-gate F2 handoff:
...
| `issue-35-curvature` / `MorganTianLib/Ch01/{Curvature,Curvature/Tensoriality}.lean` | Connected the exact bundled Levi--Civita connection to the source-ordered extension-based curvature commutator and `(0,4)` pairing; added smooth-section regularity, first-pair skew, the local-frame chart derivative/quadratic formula, pointwise additivity/scalar laws with `TensorialAt` witnesses in all three `(1,3)` slots, and the extension-based first Bianchi identity in both `(1,3)` and `(0,4)` orders | Accepted roadmap baseline `598b3f4337666d633c3686e1e561f739f71a0e98`; Morgan--Tian Definition 1.4 and Claim 1.5, pp. 37--38; pinned Mathlib `CovariantDerivative`, `FiberBundle.extend`, local-frame, chart-derivative, torsion, and Jacobi APIs; contributor declarations in the two listed modules | Keep S06 partial until the raw local-frame/chart component is identified with the extension-based pointwise value and Euclidean/nonconstant-metric regressions are added.  Record the first-pair, tensorial, and first-Bianchi subset of S07 under the explicit `Curvature.Tensoriality` namespace; leave metric last-pair/pair interchange, differential/second Bianchi, and downstream geometric contractions to their dependency milestones |
...
| `issue-48-f1-f2-j1-integration` / `7a887d996848cf619dde9d6f6d5ca0d2d089e605` | Integrated the F1/F2/J1 boundary through `Connection.{Christoffel}`, `Curvature.{Model,Tensoriality}`, `Geodesic`, `Geodesic.HopfRinow`, `Jacobi`, and the `Ch01.lean` umbrella; moved the connection-free sign kernel to `Curvature.Model`, kept the dependent manifold commutator in `Curvature`, placed the interval `isGeodesicOn` contract in the lower geodesic layer, and exposed source-constrained Jacobi, variation, and conjugacy interfaces.  The protected-branch merge also carries the direct A2 `ChangeOfVariables` leaf, while GitHub Lean CI now selects source rebuilds with both hosted caches disabled | Morgan--Tian Definitions 1.4, 1.17, 1.19 and Theorem 1.18, printed pp. 37--44 (`morganTian2007`); do Carmo Chapters 3, 5, and 7 and Lee Theorem 6.19 as recorded cross-checks; pinned Mathlib `CovariantDerivative`, tangent/chart, `IntegralCurve.ExistUnique`, interval, and Riemannian path/distance APIs at `520045ab14e26149ee970e2e617ca04b09bde5d6`; rechecked the import/ownership graph, exported signatures, and `.github/workflows/lean.yml` options `use-mathlib-cache: false` and `use-github-cache: false` | Keep Mathlib `Manifold.riemannianEDist` and `Manifold.pathELength` as the sole stable distance/length vocabulary: migrate `canonicalRiemannianEDist` and `canonicalPathELength` out of public S19 types (or keep any temporary adapter private/direct-only) and delete it after a named downstream consumer and migration trigger.  Preserve the explicit metric, completeness, and boundaryless assumptions.  Keep S18/S19 continuation, genuine interval maximality, minimizer producers, and chart-transfer/ODE work partial; keep S21/S22 chart-transfer, ODE existence/uniqueness, variation-commutation, and nondegenerate source-segment obligations as explicit debt with their roadmap replacement triggers.  Retain `Curvature.Model`/`Curvature` ownership and the no-duplicate metric/connection route; record the GitHub source-rebuild choice as intentional, with no project-cache dependency |
| `issue-48-review-response` / `2e55eba7979c34afb29faff12e3280667c2cd5b6` | Applied the review corrections on the integrated F1/F2/J1 boundary: removed the public distance/length aliases from S19 types; made maximal lifetimes and continuation domains open and order-connected with an explicit `0`-time and overlap/initial-data contract; replaced the private geodesic theorem constant with `chartConnectionContraction`; moved Jacobi coordinate certificate functions behind the documented `Jacobi.Internal` API; and made the source-facing Jacobi segment predicate require `a < b`, propagating that invariant through variation and conjugacy projections.  The documentation-only cleanup also added the missing `morganTian2007` citation and removed the unused curvature local. | Morgan--Tian Definitions 1.17--1.19 and the Chapter 1 geodesic/Jacobi discussion, pp. 41--44 (`morganTian2007`); pinned Mathlib `Set.OrdConnected`, interval, tangent/chart, covariant-derivative, and Riemannian distance/path APIs at `520045ab14e26149ee970e2e617ca04b09bde5d6`; direct dependency-ordered Lean elaboration and exported-signature checks found no private names or duplicate distance/length vocabulary; `.github/workflows/lean.yml` was rechecked with `use-mathlib-cache: false` and `use-github-cache: false` | Accept the direct Mathlib distance/length route and the nondegenerate open interval contracts.  Keep `Jacobi.Internal` as a stable, documented coordinate-certificate boundary until chart-transfer and intrinsic ODE producers replace it; retain S18/S19 maximal-domain, continuation, minimizer, and S21/S22 chart-transfer/ODE/variation producer debt with the existing replacement triggers.  Preserve the historical `issue-48-f1-f2-j1-integration` row and the source-rebuild workflow choice; no new metric, connection, or curvature facade is introduced |
| `issue-48-protected-main-rebase` / `eeddf98a936c1b16a4024869a7b38fad23184c40` | Rebased the review-fixed F1/F2/J1 boundary onto the protected mainline tensor-Laplacian locality slice without changing the reviewed geodesic, Hopf--Rinow, or Jacobi declarations; retained the direct Mathlib distance/length vocabulary, open/order-connected domain guards, stable `Jacobi.Internal` certificate adapters, and `a < b` source-segment invariant.  The protected workflow's current policy retains hosted Mathlib artifact restoration while disabling the whole-project cache. | Rechecked the merged dependency/import graph, `MorganTianLib.Ch01.Connection.TensorLaplacian` locality boundary, pinned Mathlib interval/connection/tangent APIs at `520045ab14e26149ee970e2e617ca04b09bde5d6`, and `.github/workflows/lean.yml` options `use-mathlib-cache: true` and `use-github-cache: false`; the historical `issue-48-review-response` row remains scoped to its source-rebuild revision | Keep the review fixes and canonical representation unchanged after rebase; retain the direct-only tensor-Laplacian and S18/S19/S21/S22 producer debt with existing replacement triggers.  Treat the current protected-branch cache policy as authoritative and preserve all earlier audit rows |
