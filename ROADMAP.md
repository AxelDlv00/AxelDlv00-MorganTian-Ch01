...
| `issue-43-canonical-length-adapters` / `7ac15c791bfd0e566fe823b65f699fcdce7f3ab3` | Transported the interval Cauchy--Schwarz energy/length inequality and its continuous constant-speed equality characterization from the real speed integral to Mathlib's canonical `Manifold.pathELength`, with `[0,1]` and degenerate-interval forms; added a flat affine-line path-length regression and retained explicit `ENNReal.ofReal` nonnegativity hypotheses | Morgan--Tian energy paragraph, pp. 41--43 (`morganTian2007`); do Carmo Ch. 9 (`doCarmo1992`); pinned Mathlib `PathELength`, `ENNReal.ofReal_pow`, `ENNReal.ofReal_le_ofReal`, and `ENNReal.ofReal_eq_ofReal_iff` APIs at `520045ab14e26149ee970e2e617ca04b09bde5d6`; isolated pinned-toolchain elaboration and exported axiom probes | Keep S20 **Partial**. This is an analytic canonical-length adapter only: no geodesic equation/converse, minimizer, or S19 path-metric identification is claimed; retain the existing variation-local acceleration boundary until S18 supplies the intrinsic producer |
