...
| S20 | Energy, first variation, criticality, constant speed, and energy/length inequalities: pp. 41--43; `morganTian2007`; `doCarmo1992`, Ch. 9, pp. 185--201 | `Geodesic.energy`, `Geodesic.Variation.firstEnergyVariation` | F2 | S01--S02 and S18 | exact path/variation regularity, endpoint terms, and equality conditions must remain visible | **Partial.** `Ch01.Geodesic.Variation` exports canonical bundle-metric energy and length contracts, interval Cauchy--Schwarz/additivity, positive affine reparameterization, smooth two-parameter variations, endpoint first variation, and explicit fixed-endpoint criticality/one-way zero-acceleration implications. The universal criticality converse, intrinsic S18 acceleration/geodesic producer, and minimizer notions remain deferred. |
...
| `issue-48-documentation-response` / `461b6d794da2c050f517211e3d66d345b2167520` | Restored the historical `G2-curvature-kernel` row to its revision-local `MorganTianLib/Ch01/Curvature.lean` path; added the standard Apache file headers and per-field docstrings to the introduced F1/F2/J1 modules; and keyed the Hopf--Rinow cross-checks as `doCarmo1992` (Chapter 7, Section 2) and `lee2018` (Theorem 6.19) | Base-snapshot comparison for the historical row; `docs/references.bib` entries `doCarmo1992` and `lee2018`; exact declarations in `MorganTianLib/Ch01/Geodesic/HopfRinow.lean` | Preserve all earlier audit rows verbatim, including the historical `issue-48-f1-f2-j1-integration` wording; use the keyed module citation and this append-only correction row for the current documentation record |
