...
it explicitly disables both the action's hosted Mathlib artifact restoration
and its whole-`.lake` GitHub cache, so the pinned toolchain rebuilds
dependencies from source and the build has no project-cache dependency on the
Actions cache service.  The separate
