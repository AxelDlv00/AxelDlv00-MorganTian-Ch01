...
first-order arbitrary-extension producer remains a named S06/S07 replacement
target; the private chart calculation is kept out of this umbrella.  The
public curvature consumers include the algebraic sectional-curvature and
intrinsic-plane layers and the symmetric second-exterior-power operator.  Their
direct-only tangent adapters are exported by
...
for compatibility and dependency transparency, and these adapters do not
replace the provisional producer.  It also exposes the algebraic curvature-convention
